...
- The user-authorized post-release WM-v0 P0/P0.1/P0.2 native ACT line is complete. P0 remains an immutable rejected unbounded baseline. P0.1 preserved native action validity but achieved 0/30 successes and zero grasps, and remains frozen under its historical Result B label. P0.2 found no temporal-alignment defect, removed the global gripper endpoint collapse through bounded phase/gripper-aware supervision, and selected execution horizon 2 plus checkpoint step 18,000 without final-seed access. On the fixed 30 development seeds it achieved 28 pre-grasp entries, 20 valid near-object closes, one verified grasp that dropped before lift, zero lifts, zero successes, and zero action/simulator/workspace integrity failures. P0.2 is frozen as Result C because the 18/30 grasp and 75% promotion gates failed. Final seeds `900000..900099` remain sealed, no `PolicyPackage` exists, the D2 registry has zero accepted compatible policies, and the D2 policy-generated pilot remains blocked unless a later separately authorized policy passes a new frozen gate.
