...
- The sole user-authorized post-release exception WM-v0 P0/P0.1 native ACT is complete. P0 remains an immutable rejected unbounded baseline. P0.1 completed the same ACT configuration with versioned end-to-end `affine_tanh_v1` action parameterization and zero development action-boundary violations, but its selected checkpoint achieved 0/30 successes and zero grasps. It is frozen as Result B: the 75% promotion gate failed, final seeds `900000..900099` remain sealed, no `PolicyPackage` exists, the D2 registry has zero accepted compatible policies, and the D2 policy-generated pilot remains blocked unless a later separately authorized policy passes a new frozen gate.
